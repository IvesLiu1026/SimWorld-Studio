...
# PATCH: this editor build's official MCP intermittently accepts a call but doesn't execute it
# (returns empty pythonLogs) while it is busy with the post-boot asset scan. Every pipeline script
# prints a marker (STAGE_READY / FACTS / SPAWN_FAIL / NO_GEOMETRY / SHOT), so empty logs == the call
# didn't land -> retry until it does. This wraps the module-level ue() that spawn_and_measure /
# render_views / SETUP_STAGE all call.
import time as _t_patch, json as _json_patch, threading as _thr_patch
_orig_ue = runner.ue
_DBG = "/tmp/retry_ue_debug.log"
def _dbg(msg):
    try:
        with open(_DBG, "a") as _f:
            _f.write(f"[{_thr_patch.current_thread().name}] {msg}\n")
    except Exception:
        pass
def _retry_ue(script, port, timeout=120):
    resp = _orig_ue(script, port, timeout)
    _dbg(f"port={port} script0={script[:45]!r} -> logs={runner.logs(resp)!r} raw={_json_patch.dumps(resp)[:200]}")
    for _ in range(40):
        if runner.logs(resp):
            return resp
        ...
        _t_patch.sleep(15)
        resp = _orig_ue(script, port, timeout)
        _dbg(f"RETRY port={port} script0={script[:45]!r} -> logs={runner.logs(resp)!r} raw={_json_patch.dumps(resp)[:200]}")
    return resp
runner.ue = _retry_ue
# NOTE: no SETUP_STAGE registry scan — the one-time boot rescan already registers the new pack
# (it writes a fresh CachedAssetRegistry), and scanning inside SETUP_STAGE hung the call.
...
def launch_workers(args: argparse.Namespace, run_dir: pathlib.Path) -> list[dict[str, Any]]:
    ...
    for worker in workers:
        ...
        # PATCH: this editor build returns empty pythonLogs over MCP; the stdio adapter recovers
        # output by scraping LogPython from the editor log IF SIMWORLD_EDITOR_LOG is set. launch_bridge
        # inherits os.environ, so point it at THIS worker's live editor log before spawning the bridge.
        os.environ["SIMWORLD_EDITOR_LOG"] = str(pathlib.Path(worker["project_dir"]) / "Saved" / "Logs" / "SimWorld.log")
        ...
        # PATCH: the worker's official MCP accepts calls before the post-boot asset scan finishes,
        # so SETUP_STAGE can return empty python_logs. Retry with warm-up until STAGE_READY.
        stage = None
        for _attempt in range(18):
            stage = runner.ue(runner.SETUP_STAGE, int(worker["mcp_port"]), 90)
            if "error" not in stage and any("STAGE_READY" in line for line in runner.logs(stage)):
                print(f"stage setup ok on worker {worker['worker_idx']} after {_attempt+1} attempt(s)", flush=True)
                break
            time.sleep(10)
        if not stage or "error" in stage or not any("STAGE_READY" in line for line in runner.logs(stage)):
            raise RuntimeError(f"stage setup failed on worker {worker['worker_idx']} after retries: {stage}")
        boot_rows.append({
