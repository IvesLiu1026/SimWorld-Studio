...
- [x] **T1A.10c [Correctness]** 將 provider intent summarizer 納入同一份 aggregate Review budget：執行前 gate、run-remaining CLI cap、usage/cost snapshot、Text/Visual parity，以及 provider-attempt accounting 不明時在 builder mutation 前 fail closed。
...
Implementation note（2026-07-22）：T1A.8 的 conversation/run cancellation、builder/critic/summarizer/capture abort，以及 intent/round/session isolation 已完成。T1A.10c 現將 summarizer、builder、critic 三階段記入同一份 aggregate snapshot；summarizer 在 builder／Visual capture 前接受 run-remaining gate 與向下取整的 CLI cap，只有可證明 provider 尚未 spawn 時才能 deterministic fallback。可能已執行但 accounting 不明會 fail closed；已知付費失敗或超支仍先入帳。Claude terminal usage/cost 在 close、abort、timeout、process error 競態下 exactly-once 提交，包含無尾端換行的完整 JSON；callback 或 protocol 異常保持 fail closed。Focused Review tests 101/101、完整 backend tests 733/733、Vite build 通過，獨立唯讀複審未發現剩餘 P0/P1。T1A.10 的 HTTP `/api/chat` coordinator harness 另覆蓋 Text/Visual success、401、429、500、timeout、malformed SSE、builder failure 不叫 critic、Visual-only capture、exact cancel 與跨 lease isolation。Production review scope 由 server-side active Studio lease 權限衍生。Durable outbox、managed opaque evidence lifecycle 已完成離線驗證，但尚未 provision Production journal/evidence root、service-scoped Claude auth、retention/cleanup/restart 或 backup/restore；T1A.11 仍需在 disposable scene 保存真實 provider/model/usage/verdict 與 Visual scene diff。
...
Implementation note（2026-07-21）：T1B.2、T1B.5～11b 的 code-only contract 已完成。Local/AWS profiles 共用 pinned-image、loopback、file-secret、catalog/model mount 與 embedding service contract；offline preflight、model artifact manifest、backup bundle及 live-audit receipt 都 fail closed。Sealed pending-job preparation只封裝輸入與approval basis，不會自行建index。新的semantic v2離線checkpoint已閉合21個release-pinned formal schemas（含signed launcher verification receipt）、20個unregistered adapter schema digests與24個closed-runtime schemas，並具備exact one-use handoff binding、durable ordinary/control ledgers、isolated process-group deadline/kill/reap，以及authenticated phase/control commit-before-send service。六方 approval 具備真實 Ed25519 verifier及opaque aggregate，但aggregate尚未簽入launcher/state/terminal，phase evidence仍缺detached worker signature。Production adapter registry仍為空；七個real executors、fixed worker dispatch、reproducible worker image/source closure、root-capability handoff evidence與live target observation仍未完成。`/health/ready` 在沒有正式 `simworld-asset-snapshot/v1`、一致 counts 與已驗證 revision 時仍明確 not-ready。T1B.1／3／4／12 仍受 Data/Admin/State-change Gate 約束：目前未取得 authoritative full catalog，沒有啟動Postgres/Qdrant/embedding/model service，沒有執行schema/catalog/embedding/index或live audit，也沒有修改 UE scene。
...
- [ ] **T3.5b [UE Content Evidence]** 將read-only filename-only candidates轉為derived/verified profile。UE 5.3.2 NullRHI已證12/12個animation-stack來源objects可load，並驗證pawn mesh／AnimBP／skeleton、兩個pick-up montage及`MM_Fall_Loop` clip屬性；receipt為`derived_verified_loadable`。但它們仍是archive source、不是project-owned `/Game/VISTA/MMG040` content；Rig preview mesh與pawn不完全相同、14個PoseAsset stale warnings加上一個inspection-script deprecation warning、attachment notify／landed fall／live playback未證，因此current Production profile仍維持`runtime_ready=false`、`start_allowed=false`。
