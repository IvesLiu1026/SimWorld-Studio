export const STATIC_MCP_TOOL_DEFS = [
  ...
  { id: "verify_scene", name: "verify_scene", mcpName: "verify_scene", enabled: true, description: "Run the configured visual verifier against the current scene. Captures the viewport and actor inventory, then checks placement against the scene specification. Returns structured status, issues, and corrective suggestions.", paramsSchema: { type: "object", properties: { original_request: { type: "string", description: "The scene specification to verify against (e.g. 'a suburban street with 3 houses and 2 trees')" }, focus_areas: { type: "string", description: "Optional aspects to review (e.g. 'building spacing' or 'tree placement')" } }, required: [] } },
