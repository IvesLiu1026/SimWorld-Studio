...
// In-memory registry of background python jobs (only those that exceed the
// synchronous window survive here for polling via get_python_result).
const _pyJobs=new Map();
function _extractTraceback(logs){
  const joined=(Array.isArray(logs)?logs:[]).join("\n");
  const i=joined.indexOf("Traceback (most recent call last)");
  return i>=0?joined.slice(i):"";
}
function _finalizePy(r){
  const logs=(r&&r.result&&r.result.python_logs)||[];
  // UE often returns status:"success" even when the script raised — the uncaught
  // exception only shows up as an "Error: Traceback ..." block in python_logs. So we
  // treat a traceback in the logs as failure regardless of the status field; otherwise
  // the agent is told "success" while the scene silently didn't build.
  const tb=_extractTraceback(logs);
  const ok=r?.status==="success"&&!tb;
  return{ok,logs,tb,msg:(r&&(r.message||r.error||(r.result&&r.result.error)))||""};
}
// Robust python exec: wait up to ~50s for completion. The vast majority of
// scripts — and ALL errors (which surface fast) — finish in that window, so the
// agent gets the full result INLINE, including the traceback, and can self-correct
// in one step. Only genuinely long builds (>window) fall back to a job_id the
// agent polls with get_python_result. We never make the agent read a log file.
async function toolExecutePython({script:e,wait_seconds:w}){
  ...
  const job={state:"running",python_logs:[],traceback:"",error:"",started:Date.now()};
  _pyJobs.set(jobId,job);
  const done=ueCommand("execute_python_script",{script:e},300000).then(r=>{
    const f=_finalizePy(r);
    job.state=f.ok?"done":"error";job.python_logs=f.logs;job.traceback=f.tb;
    job.error=f.ok?"":(f.tb||f.msg||"Python execution failed");job.finished=Date.now();
    const tail=f.ok?`\n[DONE] ${new Date().toISOString()}\n`:`\n[ERROR] ${new Date().toISOString()}\n${f.tb||f.msg}\n`;
    try{fs.appendFileSync(logPath,f.logs.join("\n")+tail,"utf-8")}catch(_){}
    return f;
  }).catch(err=>{
    const msg=err?.message||String(err);
    job.state="error";job.error=msg;job.finished=Date.now();
    try{fs.appendFileSync(logPath,`\n[ERROR] ${new Date().toISOString()}\n${msg}\n`,"utf-8")}catch(_){}
    return{ok:!1,logs:[],tb:"",msg};
  });
  const budgetMs=Math.max(5,Math.min(55,Number(w)||50))*1000;
  const winner=await Promise.race([done,new Promise(res=>setTimeout(()=>res("__timeout__"),budgetMs))]);
  if(winner==="__timeout__"){
    return{status:"running",job_id:jobId,hint:`Script still running after ${budgetMs/1000}s. Call get_python_result with job_id="${jobId}" to fetch python_logs + traceback when it finishes. Do NOT issue other UE tools meanwhile — UE's command queue is serial.`};
  }
  if(winner.ok)return{status:"success",job_id:jobId,python_logs:winner.logs};
  return{status:"error",job_id:jobId,error:winner.tb||winner.msg||"Python execution failed",traceback:winner.tb,python_logs:winner.logs};
}
async function toolGetPythonResult({job_id:e}){
  const job=_pyJobs.get(e);
  if(!job)return{status:"error",error:`Unknown job_id "${e}" (expired or never existed).`};
  if(job.state==="running")return{status:"running",job_id:e,hint:"Still running — poll again in a few seconds."};
  if(job.state==="done")return{status:"success",job_id:e,python_logs:job.python_logs};
  return{status:"error",job_id:e,error:job.error,traceback:job.traceback,python_logs:job.python_logs};
