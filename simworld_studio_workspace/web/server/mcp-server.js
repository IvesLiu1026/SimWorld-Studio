...
async function toolSearchAssets({query:e,category:t,k:s}){try{const{searchAssets:n}=require("./asset-retrieval-db");let o=await n({query:e,category:t,k:s});o=o.filter(a=>!_mcpIsJunkAsset(a));return{status:"success",query:e||"",category:t||null,count:o.length,assets:o.map(a=>({name:a.name,path:a.path,spawn_tool:a.spawnTool,category:a.category,setting:a.setting,dims:a.dims,desc:a.desc}))}}catch(n){return{status:"error",message:String(n&&n.message||n)}}}
