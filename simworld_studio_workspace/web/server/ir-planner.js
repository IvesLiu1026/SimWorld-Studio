...
function buildPlannerPrompt(scene, assets, brief, rich) {
  ...
  return [
    ...
    '      "facing": "<anchor/object id this object should FACE — set it for stalls, benches, vendors, doors, vehicles, signage, lamps: face the walkway/plaza/street/focal it belongs to>", "rotation_deg": <optional explicit yaw if no facing target> }',
    '  ],',
    ...
    "- ORIENTATION MATTERS (do not skip this): give functional objects a `facing` (or explicit `rotation_deg`) so they face SENSIBLY — market stalls/benches/vendors/carts face the walkway or plaza they serve; building fronts & doors face the nearest street/square; vehicles align along lanes; lanterns & signage face the path. A scene where every object sits at the same default angle looks fake and machine-made — VARY orientation naturally so the place feels real. Set `facing` on most objects and patterns.",
