...
/* ── Asset-retrieval A/B palette injection (prototype; inert unless s.body.assetMode is "retrieval" | "baseline_full") ── */
try{const __am=(s.body&&s.body.assetMode)||"";if(!MOCK_MODE&&(__am==="retrieval"||__am==="baseline_full")){a("retrieval",{phase:"start",mode:__am});const __mdl=require("./model-config").resolveModel(s.body);const __blk=await require("./asset-retrieval").buildPromptBlock(t,__am,{model:__mdl,log:(x)=>logToFile("retrieval",x)});if(__blk)m=m+String.fromCharCode(10,10)+__blk;logToFile("retrieval","mode="+__am+" injected="+(__blk?__blk.length:0)+"chars");a("retrieval",{phase:"done",mode:__am,chars:(__blk?__blk.length:0)});}}catch(__re){logToFile("retrieval","ERR "+(__re&&__re.message));}
