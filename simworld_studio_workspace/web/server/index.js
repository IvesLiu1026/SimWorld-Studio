...
Feedback: ${i}`);if(process.env.SCENE_AGENT==="opencode"){
  a("system",{sessionId:STUDIO_SESSION,mcpServers:[{name:"simworld",status:"connected"}]});
  logToFile("opencode",`User: "${t.slice(0,200)}" sessionId=${n||"new"}`);
  try{fs.writeFileSync(path.join(LOG_DIR,"raw_latest.jsonl"),"")}catch{}
  const oprompt=m+"\n\nUser request: "+t;
  const args=["run","-m",process.env.OPENCODE_MODEL||"opencode/deepseek-v4-flash-free","--format","json","--dangerously-skip-permissions",oprompt];
  const h=Object.assign({},process.env);
  const g=spawn(OPENCODE_BIN,args,{cwd:path.resolve(__dirname,".."),env:h,stdio:["ignore","pipe","pipe"]});
  _chatProcs.set(n||"_global",g);
  g.on("exit",()=>_chatProcs.delete(n||"_global"));
  let buf="",latest=null,stderrBuf="";
  function recentShot(){
    if(latest)return `/api/screenshot/file?path=${encodeURIComponent(latest)}`;
    try{
      const files=fs.readdirSync(SCREENSHOT_DIR)
        .filter(f=>f.endsWith(".png"))
        .map(f=>({fp:path.join(SCREENSHOT_DIR,f),time:fs.statSync(path.join(SCREENSHOT_DIR,f)).mtimeMs}))
        .sort((x,y)=>y.time-x.time);
      if(files.length)return `/api/screenshot/file?path=${encodeURIComponent(files[0].fp)}`;
    }catch{}
    return null;
  }
  function parseLine(line){
    ...
    if(r.type==="text"&&r.part?.text){
      a("text",{delta:r.part.text});
    }else if(r.type==="tool_use"&&r.part){
      const id=r.part.callID||r.part.id||("opencode-"+Date.now());
      const name=r.part.tool||"tool";
      const displayName=name.replace(/^simworld_/,"");
      a("tool_start",{id,name,displayName});
      a("tool_details",{id,name,displayName,input:r.part.state?.input||{}});
      const out=r.part.state?.output||"";
      const mpath=String(out).match(/([\/][\w\/\-._]+\.png)/);
      if(mpath&&fs.existsSync(mpath[1])){
        latest=mpath[1];
        a("screenshot",{toolUseId:id,filepath:`/api/screenshot/file?path=${encodeURIComponent(latest)}`});
      }
      a("tool_result",{toolUseId:id,result:String(out).slice(0,2000),isError:false});
    }
  }
  g.stdout.on("data",d=>{
    buf+=d.toString();
