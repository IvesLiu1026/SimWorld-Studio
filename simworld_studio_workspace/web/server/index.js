...
const runtimeMutationArbiter = createRuntimeMutationArbiter();
const runtimeMutationMiddleware = createRuntimeMutationMiddleware({ arbiter: runtimeMutationArbiter });
...
const reviewEvidenceProbe=createReviewEvidenceReadinessProbe();
const reviewEvidenceLifecycle=Object.freeze({
  admit:({scope,mode,signal,dependencies})=>acquireReviewEvidenceAdmission({
    root:mode==="visual_loop"?DEFAULT_REVIEW_EVIDENCE_ROOTS[1]:DEFAULT_REVIEW_EVIDENCE_ROOTS[0],
    scopeId:scope.scopeId,
    ueBroker:dependencies&&dependencies.ueBroker,
    signal,
    mode,
    maxRounds:parseInt(((dependencies&&dependencies.env)||process.env).SCENE_LOOP_MAX_ROUNDS||"5",10),
  }),
  finalize:(admission,options)=>finalizeReviewEvidenceAdmission(admission,options),
});
...
const _TRUSTED_BUILDER_PROFILE=new Set(["trusted_proxy","trusted-proxy","public_webrtc","public-webrtc"]).has(STUDIO_TRANSPORT.profile);
const BROKERED_MCP_CONFIG=_TRUSTED_BUILDER_PROFILE
  ?require("./mcp-runtime-config").ensureBrokeredMcpRuntimeConfig({env:process.env,baseDir:__dirname}).path
  :MCP_CONFIG;
...
const SCENE_LOOP_DEFAULT_MODE=_normalizeMode(process.env.SCENE_LOOP_MODE)||((process.env.SCENE_LOOP_ENABLED==="1")?"text_loop":"vanilla");
const reviewModeStore=createScopedReviewModeStore({
  transportProfile:STUDIO_TRANSPORT.profile,
  defaultMode:SCENE_LOOP_DEFAULT_MODE,
});
const reviewLoopCoordinator=createReviewLoopCoordinator({
  ...
  reviewPreflight:reviewEvidenceProbe,
  reviewEvidenceLifecycle,
  mutationArbiter:runtimeMutationArbiter,
  captureReviewBinding:async({scope,signal})=>{
    const broker=scope.leaseBound
      ?createLeaseBoundReviewBroker({scope,resolveUeBroker:_resolveVistaSlotBroker})
      :ueBroker;
    if(!broker||typeof broker.send!=="function")throw new Error("Review UE broker unavailable");
    const snapshot=await broker.send("get_actors_in_level",{}, {
      timeoutMs:15000,
      queueDeadlineMs:30000,
      ...(signal?{signal}:{}),
    });
    const sceneBuildLineage=scope.leaseBound
      ?vistaSceneBuildRuntime.service.resolveActiveRuntimeLineage(scope.activeLease)
      :null;
    return createReviewSceneBinding({scope,snapshot,sceneBuildLineage});
  },
  handlerDependencies:({scope})=>({
    STUDIO_SESSION,
    logToFile,
    intentStore,
    ueBroker:scope.leaseBound
      ?createLeaseBoundReviewBroker({scope,resolveUeBroker:_resolveVistaSlotBroker})
      :ueBroker,
...
const _resolveVistaSlotUcvBroker=createVistaSlotUcvBrokerResolver({
  studioStreaming,
  defaultBroker:ucvBroker,
  BrokerClass:UcvBroker,
});
const _internalRunCapabilities=createInternalRunCapabilityRegistry({
  isActiveSessionBinding:(identity)=>studioStreaming.isActiveSessionBinding(identity),
  resolveUeBroker:_resolveVistaSlotBroker,
  resolveUcvBroker:_resolveVistaSlotUcvBroker,
  operationPolicy:(context)=>internalCapabilityOperationPolicy(context,process.env),
});
const _builderRuntimeAuthority=createBuilderRuntimeAuthority({
  transportProfile:STUDIO_TRANSPORT.profile,
  registry:_internalRunCapabilities,
  mcpConfigPath:BROKERED_MCP_CONFIG,
  serverPort:PORT,
});
function _authorizeInternalRun(request,channel){
  if(isInternalCapabilityCandidate(request)){
    return _internalRunCapabilities.authorize({
      capability:capabilityHeader(request),
      runId:runIdHeader(request),
      channel,
      body:request.body,
    });
  }
  if(_TRUSTED_BUILDER_PROFILE){
    const error=new Error("A server-issued internal run capability is required.");
    error.code="INTERNAL_RUN_CAPABILITY_REQUIRED";
    error.statusCode=401;
    throw error;
  }
  return Object.freeze({broker:channel==="ue"?ueBroker:channel==="ucv"?ucvBroker:null});
}
...
const vistaRuntimeRegistry=createVistaRuntimeControllerRegistry({
  resolveIdentity:_resolveVistaIdentity,
  resolveUeBroker:_resolveVistaSlotBroker,
  isActiveSessionBinding:(identity)=>studioStreaming.isActiveSessionBinding(identity),
...
app.post("/api/chat-stop",(req,res)=>{
  ...
  const requestedRunId=body.runId||(req.query&&req.query.runId)||null;
  const capabilitiesRevoked=!requestedRunId
    ?_internalRunCapabilities.revokeScope({scopeId:cancellation.scope.scopeId},"user_stop")
    :cancelled&&cancelled.runId===requestedRunId
      ?_internalRunCapabilities.revokeScope({scopeId:cancellation.scope.scopeId,runId:requestedRunId},"user_stop")
      :0;
  if(!requestedRunId||cancelled){
  ...
  res.json({stopped:!!cancelled||subprocessStopped||capabilitiesRevoked>0,runId:cancelled&&cancelled.runId,loopAborted:!!cancelled,subprocessStopped,capabilitiesRevoked});
});
...
// opening their own one-shot socket. Public profiles additionally require a
// server-minted, lease-bound run capability before selecting the slot broker.
app.post("/api/internal/ucv",async(req,res)=>{
  ...
  let authorized;
  try{authorized=_authorizeInternalRun(req,"ucv");}
  catch(error){return res.status(error.statusCode||401).json({ok:false,code:error.code||"INTERNAL_RUN_CAPABILITY_INVALID",error:error.message||"Internal run capability was rejected."});}
  const{cmd,timeoutMs,retries,queueDeadlineMs}=req.body||{};
  if(typeof cmd!=="string"||!cmd){
    return res.status(400).json({ok:false,error:"cmd required"});
  }
  try{
    const result=await authorized.broker.send(cmd,{
...
// Semantic search stays in the main process so database/vector/embedding
// credentials never cross into the model-driven MCP subprocess. The same
// short-lived lease capability used for UE/UCV selects this exact build run.
app.post("/api/internal/assets",async(req,res)=>{
  const ip=(req.ip||"").replace(/^::ffff:/,"");
  if(ip!=="127.0.0.1"&&ip!=="::1"&&ip!=="localhost"){
    return res.status(403).json({ok:false,error:"loopback only"});
  }
  if(!internalAssetSearchAllowed(req.body)){
    return res.status(400).json({ok:false,code:"ASSET_SEARCH_REQUEST_INVALID",error:"A bounded semantic asset query is required."});
  }
  try{_authorizeInternalRun(req,"assets");}
  catch(error){return res.status(error.statusCode||401).json({ok:false,code:error.code||"INTERNAL_RUN_CAPABILITY_INVALID",error:error.message||"Internal run capability was rejected."});}
  if(!vistaAssetRuntime.searchAssets){
    return res.status(503).json({ok:false,code:"VISTA_ASSET_RUNTIME_DISABLED",error:"Verified semantic asset search is unavailable."});
  }
  try{
    const assets=await vistaAssetRuntime.searchAssets(req.body);
    return res.json({ok:true,result:{assets:Array.from(assets),retrieval:assets.retrieval||null}});
  }catch(error){
    const failure=require("./asset-retrieval").serializeAssetRetrievalError(error);
    return res.status(503).json({ok:false,...failure});
  }
});
...
app.post("/api/internal/ue",async(req,res)=>{
  ...
  let authorized;
  try{authorized=_authorizeInternalRun(req,"ue");}
  catch(error){return res.status(error.statusCode||401).json({ok:false,code:error.code||"INTERNAL_RUN_CAPABILITY_INVALID",error:error.message||"Internal run capability was rejected."});}
...
function _unsafeCodingAgentProductionGuard(_req,res,next){
  if(!productionExecutionLocked(process.env))return next();
  return res.status(503).json({
    code:"UNSANDBOXED_CODING_AGENT_DISABLED",
    error:"Legacy coding-agent execution is unavailable under production safety policy.",
  });
}
app.post("/api/agent-chat",_unsafeCodingAgentProductionGuard);
app.post("/api/agent-broadcast",_unsafeCodingAgentProductionGuard);
app.post("/api/arena/battles/:id/run",_unsafeCodingAgentProductionGuard);
app.post("/api/arena/run",_unsafeCodingAgentProductionGuard);
...
app.get("/api/scene-loop",(req,res)=>{try{const scope=_reviewScope(req),mode=reviewModeStore.get(scope);res.json({enabled:mode!=="vanilla",mode,intentSummary:intentStore.get(scope.scopeId)||null});}catch(error){res.status(error.statusCode||400).json({error:error.message,code:error.code||"REVIEW_SCOPE_INVALID"});}});
app.post("/api/scene-loop",(req,res)=>{try{const b=req.body||{},scope=_reviewScope(req);let md=_normalizeMode(b.mode);if(md===null&&typeof b.enabled==="boolean")md=b.enabled?"text_loop":"vanilla";if(md===null)return res.status(400).json({error:"mode required: vanilla|text_loop|visual_loop"});md=reviewModeStore.set(scope,md);logToFile("scene-loop","mode set for scoped review session");res.json({enabled:md!=="vanilla",mode:md});}catch(error){res.status(error.statusCode||400).json({error:error.message,code:error.code||"REVIEW_SCOPE_INVALID"});}});
app.delete("/api/scene-loop",(req,res)=>{try{const scope=_reviewScope(req);reviewModeStore.delete(scope);intentStore.delete(scope.scopeId);res.json({ok:true});}catch(error){res.status(error.statusCode||400).json({error:error.message,code:error.code||"REVIEW_SCOPE_INVALID"});}});
...
// Legacy ad-hoc VLM scoring bypassed coordinator admission, journal, budget,
// and evidence retention. Keep an explicit migration response for old clients;
// all model-backed Review now enters through /api/chat with loopMode.
app.post('/api/vlm-score',(_req,res)=>{
  res.status(410).json({
    code:'REVIEW_COORDINATOR_REQUIRED',
    error:'Use /api/chat with loopMode text_loop or visual_loop for managed Review.',
  });
});
...
bootstrapDefaultReviewEvidenceRoots().then(()=>{
...
`)});
  ...
  console.error(`[review-evidence] startup bootstrap failed: ${error&&error.message||error}`);
  process.exitCode=1;
});
