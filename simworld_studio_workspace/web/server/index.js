...
try{const __sir=require("./scene-ir");if(!MOCK_MODE&&__sir.resolveIRMode(s.body||{})==="on"){a("ir",{phase:"start"});const __irmdl=require("./model-config").resolveModel(s.body);const __irrun=_normalizeRunner((s.body&&s.body.runner)||(s.body&&s.body.agent)||"")||_DEFAULT_RUNNER;const __irblk=await __sir.buildIRBlock(t,{model:__irmdl,provider:__irrun,runner:__irrun,irSolver:(s.body&&s.body.irSolver),irRepair:(s.body&&s.body.irRepair),irCot:(s.body&&s.body.irCot),irAscii:(s.body&&s.body.irAscii),irRichAssets:(s.body&&s.body.irRichAssets),irPlanCritic:(s.body&&s.body.irPlanCritic),log:(x)=>logToFile("ir",x)});if(__irblk)m=m+String.fromCharCode(10,10)+__irblk;logToFile("ir","injected="+(__irblk?__irblk.length:0)+"chars");a("ir",{phase:"done",chars:(__irblk?__irblk.length:0)});}}catch(__ie){logToFile("ir","ERR "+((__ie&&__ie.message)||__ie));a("ir",{phase:"error",message:(__ie&&__ie.message)||String(__ie)});}
