...
// either payload and neither payload saves content or starts PIE/SIE. Setup may
// materialize one fixed, unsaved Engine Cube as the bounded demo walk surface.
const FIXED_SETUP_SCRIPT = [
  "import json",
  "import unreal",
  `GAME_MODE_CLASS = ${JSON.stringify(VISTA_GAME_MODE_CLASS)}`,
  `PAWN_CLASS = ${JSON.stringify(VISTA_PAWN_CLASS)}`,
  "GROUND_LABEL = 'VISTA_RuntimeGround'",
  "GROUND_MESH = '/Engine/BasicShapes/Cube.Cube'",
  "GROUND_HALF_EXTENT_Z = 50.0",
  "GROUND_SCALE_XY = 10000.0",
  ...
  "capsule_component = pawn_default.get_component_by_class(unreal.CapsuleComponent)",
  "if capsule_component is None:",
  "    raise RuntimeError('fixed VISTA pawn capsule is unavailable')",
  "capsule_half_height = float(capsule_component.get_unscaled_capsule_half_height())",
  "if capsule_half_height < 1.0 or capsule_half_height > 1000.0:",
  "    raise RuntimeError('fixed VISTA pawn capsule height is invalid')",
  "player_start_location = player_starts[0].get_actor_location()",
  "ground_location = unreal.Vector(",
  "    float(player_start_location.x),",
  "    float(player_start_location.y),",
  "    float(player_start_location.z) - capsule_half_height - GROUND_HALF_EXTENT_Z,",
  ")",
  "ground_mesh = unreal.load_asset(GROUND_MESH)",
  "if ground_mesh is None or not isinstance(ground_mesh, unreal.StaticMesh):",
  "    raise RuntimeError('fixed runtime ground mesh is unavailable')",
  "runtime_grounds = [actor for actor in actor_subsystem.get_all_level_actors() if actor.get_actor_label() == GROUND_LABEL]",
  "if len(runtime_grounds) > 1:",
  "    raise RuntimeError('multiple fixed runtime grounds are present')",
  "ground = runtime_grounds[0] if runtime_grounds else actor_subsystem.spawn_actor_from_object(",
  "    ground_mesh, ground_location, unreal.Rotator(0.0, 0.0, 0.0), transient=False",
  ")",
  "if ground is None or not isinstance(ground, unreal.StaticMeshActor):",
  "    raise RuntimeError('fixed runtime ground could not be created')",
  "ground.set_actor_label(GROUND_LABEL)",
  "ground.set_actor_location(ground_location, False, False)",
  "ground.set_actor_rotation(unreal.Rotator(0.0, 0.0, 0.0), False)",
  "ground.set_actor_scale3d(unreal.Vector(GROUND_SCALE_XY, GROUND_SCALE_XY, 1.0))",
  "ground_component = ground.get_component_by_class(unreal.StaticMeshComponent)",
  "if ground_component is None:",
  "    raise RuntimeError('fixed runtime ground component is unavailable')",
  "ground_component.set_static_mesh(ground_mesh)",
  "ground_component.set_collision_enabled(unreal.CollisionEnabled.QUERY_AND_PHYSICS)",
  "ground_component.set_collision_profile_name('BlockAll')",
  "applied_ground_mesh = ground_component.get_editor_property('static_mesh')",
  "if applied_ground_mesh is None or applied_ground_mesh.get_path_name() != GROUND_MESH:",
  "    raise RuntimeError('fixed runtime ground mesh was not applied')",
  "payload = {",
