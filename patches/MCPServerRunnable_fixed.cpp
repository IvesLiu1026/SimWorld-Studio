    ...
    while (bRunning)
    {
        ...
        if (ListenerSocket->HasPendingConnection(bPending) && bPending)
        {
            ...
            if (ClientSocket.IsValid())
            {
                ...
                uint8 Buffer[8192];
                FString MessageBuffer;  // accumulate across TCP packets
                while (bRunning)
                {
                    int32 BytesRead = 0;
                    if (ClientSocket->Recv(Buffer, sizeof(Buffer), BytesRead))
                    {
                        ...
                        // Convert received data to string and accumulate
                        Buffer[BytesRead] = '\0';
                        FString ReceivedText = UTF8_TO_TCHAR(Buffer);
                        MessageBuffer.Append(ReceivedText);
                        UE_LOG(LogTemp, Display, TEXT("MCPServerRunnable: Received chunk (%d bytes), buffer now %d chars"), BytesRead, MessageBuffer.Len());
                        ...
                        {
                            MessageBuffer.Reset();  // consumed a complete message
                            // Get command type
                            FString CommandType;
                        ...
                        {
                            // Incomplete JSON — keep accumulating, more TCP packets coming
                            UE_LOG(LogTemp, Display, TEXT("MCPServerRunnable: Incomplete JSON, accumulating (%d chars so far)"), MessageBuffer.Len());
                        }
                    }
                    else
                    {
